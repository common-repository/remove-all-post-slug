    ...
    private function add_rules_htaccess() {
        ...
        if (is_readable($htaccess_filename)) {
            ...
            foreach ($wp_post_types as $type => $custom_post) {
                ...
                $rewrite_rule = (!empty($suffix)) ? "RewriteRule ^{$query_var}/(.+)/\$ {$allLang}/\$1\.{$suffix} [R=301,l]" : "RewriteRule ^{$query_var}/(.+)/\$ {$allLang}/\$1 [R=301,L]";
...
add_filter('request', function( $vars ) {
	...
	if(count($vars) == 1 && ($vars['lang'] == 'en' || $vars['lang'] == 'es') )
        {
			$olVar = $vars;
			unset($vars);
			$vars = array();
